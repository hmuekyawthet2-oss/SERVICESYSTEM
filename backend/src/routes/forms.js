...
function drawCheckbox(doc, label, checked, x, y) {
  ...
  if (checked) {
    doc.fontSize(8).text('X', x + 1.5, y + 0.5);
  }
...
// INSTALLATION FORM
// ═══════════════════════════════════════════════════════════════
router.post('/installation/pdf', (req, res) => {
  try {
    const d = req.body;
...
// MAINTENANCE FORM
// ═══════════════════════════════════════════════════════════════
router.post('/maintenance/pdf', (req, res) => {
  try {
    const d = req.body;
    const doc = new PDFDocument({ size: 'A4', margin: 50 });
    res.setHeader('Content-Type', 'application/pdf');
...
// FIELD SERVICE FORM
// ═══════════════════════════════════════════════════════════════
router.post('/field-service/pdf', (req, res) => {
  try {
    const d = req.body;
    const doc = new PDFDocument({ size: 'A4', margin: 50 });
    res.setHeader('Content-Type', 'application/pdf');
    ...
    drawCheckbox(doc, 'Maintained', (d.actions_taken || '').includes('maintained'), 165, y);
    drawCheckbox(doc, 'Reconditioned', (d.actions_taken || '').includes('reconditioned'), 260, y);
    drawCheckbox(doc, 'Replace', (d.actions_taken || '').includes('replace'), 370, y);
    drawCheckbox(doc, 'Repair', (d.actions_taken || '').includes('repair'), 450, y);
    y += 16;
    ...
    drawCheckbox(doc, 'Good', d.env_condition === 'good', 370, y);
    drawCheckbox(doc, 'Fair', d.env_condition === 'fair', 420, y);
    drawCheckbox(doc, 'Poor', d.env_condition === 'poor', 470, y);
    y += 16;
